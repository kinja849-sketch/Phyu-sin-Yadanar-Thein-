...
function pageTransitionOut() {
  ...
  if (document.querySelector('.work-single-header')) {
    tl.from($('.work-single-header .tag, .work-single-header .type'), {
      scale: 0,
      x: "8em",
      rotate: 45,
      duration: 1,
  ...
  if (document.querySelector('.home-header')) {
    ...
    tl.from($('[data-transition-words] [data-single-word]'), {
      scale: 0,
      duration: 1,
      stagger: 0.05,
      ease: "Expo.easeOut"
    ...
    tl.from($('[data-transition-words] .home-header__smiley-svg'), {
      xPercent: 100,
      rotate: 360,
      duration: 1.5,
      ease: "elastic.out(1,0.75)"
    ...
    tl.from($('[data-transition-words] .home-header__title-line-svg path'), {
      drawSVG: '100% 100%',
      duration: 1.5,
    }, "< 0.1");
    
    tl.from($('[data-transition-words] .home-header__star-inner'), {
      yPercent: 100,
      scale: 0,
      rotate: -180,
      duration: 1.5,
  ...
  if (document.querySelector('.work-header')) {
    tl.from($('.work-intro__arrow-svg path'), {
      drawSVG: '0% 0%',
      duration: 1.5,
    }, "0.8");
  }
...
function initBasicFunctions() {
  ...
  // Work Modal
  $('[data-modal-work-action="hover"]').on('mouseenter', function() {
  ...
  // WhatsApp Modal
  $('[data-modal-whatsapp-action="hover"]').on('mouseenter', function() {
    if (!window.matchMedia('(hover: none) and (pointer: coarse)').matches) {
  ...
  document.querySelectorAll('.draw-btn').forEach(function(drawBtn) {
    ...
    function runAnimation() {
      ...
      tl.to(paths[1], { drawSVG: '100% 100%', duration: dur * 0.5, ease: "Power2.easeInOut" }, dur * 0.25);
...
function initGenerateQRCode() {
    ...
    parent.querySelectorAll('[data-qr-code-canvas]').forEach(placeholder => {
      const clone = placeholder === parent.querySelector('[data-qr-code-canvas]') ? svg : svg.cloneNode(true);
...
function initMWG11() {
  ...
  document.fonts.ready.then(function() {
    $('.horizontal-words').each(function() {
      ...
      var $arrow    = $container.find('.horizontal-words__arrow-svg path, .horizontal-words__arrow-end-svg path');

      // if the main text element isn't found, skip this container
      if (!$text.length) {
        console.warn('initMWG11: .horizontal-words__relative element not found in one of the containers');
        return;
      }
      ...
      var scrollTween = gsap.fromTo($text, {
        xPercent: 50
      }, {
        xPercent: -100,
        ease: 'none',
...
function initFlickCards() {
  ...
  slides.forEach(s => { if (s) io.observe(s.iframe); });
  ...
  Draggable.create(dragger, {
    ...
    onRelease() {
      ...
      const old = slides[prevIdx], nw = slides[activeIdx];
  ...
  function loadNext(index = 0) {
    ...
    player.on('play', () => {
      ...
      Promise.all([player.getVideoWidth(), player.getVideoHeight()]).then(([videoW, videoH]) => {
        ...
        const finalW = videoRatio > wrapperRatio
          ? wrapperH * videoRatio
          : wrapperW;
        const finalH = videoRatio > wrapperRatio
          ? wrapperH
          : wrapperW / videoRatio;
